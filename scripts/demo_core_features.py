...
def demo_ai_predictions(base_url: str, token: str):
    ...
    payload = data.get("sensor_data") if isinstance(data, dict) else data
    if payload is None:
        raise RuntimeError("AI demo data missing 'sensor_data' list")
    result = call_endpoint(
        base_url, token, "/api/v1/ai/predictions/advanced-defect", payload
    )
